...
This is discussed in our textbook \cite{zybooks-ds2026} in section 3.6 ``Analyzing the time complexity of recursive algorithms''. What is the reasoning? For an input of size \verb|n|, binary search can be analyzed into two cases, the base case and the recursive case. In the base case, the position is found or is determined to be absent from the array. That requires a single call to the binary search function. In the recursive case, binary search is called on half of the list in the current scope, i.e., $n/{2}^1$, $n/{2}^2$, $n/{2}^3$, and so forth, as the function is called in each step of the recursion. The number of these recursive calls is dependent on the size of $n$. Since the size of the sublist that is searched is halved each time that the binary search function is called, $\log_{2}n$ calls are required for the recursive case. Therefore, presuming a correct implementation and that the input list is sorted, for a sorted input list of length $n$, there are $\lfloor 1 + \log_{2}n \rfloor$ calls to binary search required in the worst case to find the item or to determine that it is not in the list. Why $\lfloor 1 + \log_{2}n \rfloor$ calls and not $1 + \log_{2}n$ calls? When the input list has a size that is not a power of $2$, the $\log_{2}n \not \in \mathbb{Z}$. But the number of steps taken by the algorithm must be some $x \in \mathbb{Z}^+$. In light of this, we round down here to the highest integer less than the actual value of $1 + \log_{2}n$. See the script below to validate this empirically.
...
\begin{itemize}
  ...
  \item \verb|a(m, n) = a(m-1, a(m, n-1)) if m != 0, n != 0|
\end{itemize}
