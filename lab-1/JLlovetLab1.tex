...
I used test-driven development with unit tests written in \texttt{unittest} to help with the development of my code and to ensure that I did not break portions of the codebase as I built new features, performed refactors, and did other chores. My test writing strategy is drawn from sources like \cite{JamesLearnGoWithTests}, where I learned to write tests to specify an API contract for a package, set of functions, or entire program before implementing the features themselves. As I added features to this lab, I wrote the tests first, mocking out the functions that I would call, and then I implemented the features. There are a few small functions that I extracted later on that are not covered by the tests. So, there is not 100\% coverage for the codebase. However, the core features and workflows were already under test when those functions were written. In a further development of this lab in an industry context, I would add tests to explicitly articulate the precise specifications of the functions' expected behavior and increase the coverage of the codebase overall. As a methodological note, when developing the tests the steps I took were:
...
I referenced several resources to get back up to speed with testing in python using \texttt{unittest}. \parencite{PythonDocumentationUnittestUnit, CodewithJosh2024HowToTest, CoreySchafer2017PythonTutorial, ArjanCodes2025HowToWriteGreatUnitTestsInPython}
