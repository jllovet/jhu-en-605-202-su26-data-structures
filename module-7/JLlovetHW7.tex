...
In the implementation that I have above, the performance is quite poor, rivaling bubble sort. I suspect there may be a clever way to approach this implementation, like with a heap, whiuch might open up the ability to optimize the performance significantly. Ultimately, the core weakness of this implementation is that the analysis in the step where we tally how many elements are less than some given element must be calculated for each pair of numbers. A viable other solution would need to arrange the elements in a structure where transitivity or some other characteristic could be exploited along with a mechanism for counting. In the current implementation, there is no impact whether the data is sorted, randomized, or reversed.
...
Didn't have time. Will come back to this.
